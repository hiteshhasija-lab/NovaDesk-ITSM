...
function attachRoutes(router, entityType, { table, getEntity, canAccess, canManage, middleware = [] }) {
  ...
  router.post('/:id/attachments', ...middleware, (req, res) => {
    upload.single('file')(req, res, async (err) => {
      ...
      await logActivity(entityType, req.params.id, req.session.user.id, `Attached file: ${req.file.originalname}`);
      res.redirect(`/${table}/${req.params.id}`);
    });
  });

  router.get('/:id/attachments/:attId/download', ...middleware, async (req, res) => {
    const entity = await getEntity(req.params.id);
    if (!entity) return res.status(404).render('error', { title: 'Not Found', message: 'Record not found.' });
    if (!canAccess(req, entity)) return res.status(403).render('error', { title: 'Access Denied', message: 'You cannot view this file.' });

    const att = await db.prepare('SELECT * FROM attachments WHERE id = ? AND entity_type = ? AND entity_id = ?')
  ...
  router.post('/:id/attachments/:attId/delete', ...middleware, async (req, res) => {
    const entity = await getEntity(req.params.id);
    if (!entity) return res.status(404).render('error', { title: 'Not Found', message: 'Record not found.' });
    ...
    await logActivity(entityType, req.params.id, req.session.user.id, `Removed attachment: ${att.original_name}`);
    res.redirect(`/${table}/${req.params.id}`);
  });
}
...
function watchRoutes(router, entityType, { table, getEntity, canAccess, middleware = [] }) {
  router.post('/:id/watch', ...middleware, async (req, res) => {
    const entity = await getEntity(req.params.id);
    if (!entity) return res.status(404).render('error', { title: 'Not Found', message: 'Record not found.' });
    if (!canAccess(req, entity)) return res.status(403).render('error', { title: 'Access Denied', message: 'You cannot watch this record.' });

    await db.prepare(`INSERT INTO watchers (entity_type, entity_id, user_id) VALUES (?, ?, ?) ON CONFLICT DO NOTHING`)
      .run(entityType, req.params.id, req.session.user.id);
    res.redirect(`/${table}/${req.params.id}`);
  });

  router.post('/:id/unwatch', ...middleware, async (req, res) => {
    await db.prepare(`DELETE FROM watchers WHERE entity_type = ? AND entity_id = ? AND user_id = ?`)
      .run(entityType, req.params.id, req.session.user.id);
    res.redirect(`/${table}/${req.params.id}`);
  });
}

async function getWatchers(entityType, entityId) {
  return db.prepare(`
...
async function isWatching(entityType, entityId, userId) {
  return !!(await db.prepare(`SELECT 1 FROM watchers WHERE entity_type = ? AND entity_id = ? AND user_id = ?`)
    .get(entityType, entityId, userId));
}

// Emails every watcher except excludeUserId (typically the actor who triggered the update).
async function notifyWatchers(entityType, entityId, { subject, html, excludeUserId }) {
  const watchers = await getWatchers(entityType, entityId);
  watchers
...
async function purgeCollabData(entityType, entityId) {
  const atts = await db.prepare('SELECT * FROM attachments WHERE entity_type = ? AND entity_id = ?').all(entityType, entityId);
  atts.forEach(a => fs.unlink(path.join(uploadRoot, entityType, a.filename), () => {}));
  await db.prepare('DELETE FROM attachments WHERE entity_type = ? AND entity_id = ?').run(entityType, entityId);
  await db.prepare('DELETE FROM watchers WHERE entity_type = ? AND entity_id = ?').run(entityType, entityId);
}
